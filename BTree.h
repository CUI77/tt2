...
result SearchBTree(BTree T, int k);//在m阶B数T上查找关键字k，返回p{pt,i,tag}
int Insert(BTree &T, int k, BTree q, int i, Record *recptr);//在B树T上结点p->pt的key[i]和key[i+1]之间插入关键字k
int Remove(BTree &T, int k);//初始条件:B树上p结点存在 操作结果:删除B树T上结点p->pt的关键字k
int Menu();//输出选择菜单
void ShowBTree(BTree &p, int deep = 1);
...
int Insert(BTree &T, int k, BTree q, int i, Record *rec)
...
// 而将双亲结点中小于(或大于）且紧靠该关键字的关键字下移至q中,并返回OK，否则返回EREOR。
{
	...
	else return FALSE;							// 无关键字大于(m-1)/2的兄弟
	q->keynum++;
	b->keynum--;
...
// 如T上不存在关键字k，则返回ERROR。
{
	...
	if (res.tag == 0) return FALSE;				// 未搜索到
	else
	{
		q = res.pt;								// q指向待删结点
		i = res.i;
...
void ShowBTree(BTree &p, int deep)
{
	if (p == NULL) { return; }
	int i;
	for (i = 0; i < p->keynum; i++)
	{
		ShowBTree(p->ptr[i], deep + 1);
